...
function create() {
    ...
    // Add player and enemy groups to HitDetectionEngine to allow it to track them
    HitDetectionEngine = new HitDetection(game, playersGroup, dudes);
...
function update() {
  Debugger.showHitboxes(game, playersGroup, dudes);
  Debugger.showSpriteInfo(game, player.sprite);

  player.update();
  HitDetectionEngine.update();
