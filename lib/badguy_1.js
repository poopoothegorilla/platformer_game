...
export default class Thug {
  ...
  createEvents() {
    this.ThugEvents.on('isPunched', (puncher) => {
      console.log(this.name + ' is punched by '+puncher);
      this.injured();
    });
  }
  ...
  punch(){
    console.log(`firing ${this.name} punch`)
    this.busy = true;
    ...
    // detect if this punch overlaps a player hitBox
    console.log(this.player)
    console.log('damage player by 10 hp')
    // if so, emit isPunched event to that player's Events obj
  }
