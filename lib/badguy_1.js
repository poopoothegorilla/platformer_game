...
export default class Thug {
  ...
  create(id){
    ...
    console.log(`printing body for ${this.name}`)
  ...
  update(){
    ...
    if (!this.dazed()) {
      ...
      if (proximityDetection(this.game, this.sprite, this.player.sprite) && (this.state == 'stalking' || this.state == 'chasing')) {
  ...
  injured(damage, knockback){
    console.log(`firing injured on ${this.name}`)
    this.sprite.busy = true;
    this.state = 'injured';
    // this.sprite.body.velocity.x = 0;
    // this.sprite.body.velocity.y = 0;
