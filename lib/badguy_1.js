...
import * as HitDetection from './helpers/hit-detection.js';
...
export default class Thug {
  ...
  createEvents(game, player) {
    var ThugEvents = new EventEmitter(game, player);
    // Bind listeners
    ThugEvents.on('damagePlayer', function() {
      doDamage(player, 10)
    });
  }
  ...
  create(){
    ...
    this.createEvents(this.game, this.player);
  }
  ...
  punchHitDetection(badguy, player) {
    return HitDetection.punchHitDetection(badguy, player)
  }

  proximityDetection(badguy, player) {
    return HitDetection.proximityDetection(badguy, player)
  }

  inZpace(close, far, target) {
    return inZpace(close, far, target)
  }
