export default class HitDetection {
  ...
  checkIfAttacking(playerSprite, enemySprite) {
    if (enemySprite.state == 'attacking') {
  ...
  hitDetected(playerSprite, enemySprite) {
    console.log('firing hitDetected')
    console.log(playerSprite.events)
  ...
  update() {
    ...
    // don't forget to update group here when something dies
  }
}
