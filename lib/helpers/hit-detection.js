export default class HitDetection {
  ...
  update() {
    ...
    this.game.physics.arcade.overlap(
      ...
      this.targetGroup,
      this.registerHit,
      this.processOverlap, // If overlap is true, this callback is run to do additional checks. If checkIfAttacking in turn returns true, then registerHit is run.
      this
    )
  }

  processOverlap(attackbox, targetSprite) {
    console.log(`${attackbox.name} hit ${targetSprite.name}`)
